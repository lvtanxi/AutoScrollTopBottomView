...
public class AutoScrollTopBottomView extends RelativeLayout {
    ...
    public void autoMove() {
        if ( mScroller.getCurrY() != 0) {//prepare scroll to top
            mScroller.startScroll(mTopView.getScrollX(), mTopView.getScrollY(), 0, -mTopView.getScrollY(), ANI_TIME/2);
        } else {//prepare scroll to bottom
            mScroller.startScroll(mTopView.getScrollX(), mTopView.getScrollY(), 0, -mTopView.getScrollY() - mBottomView.getMeasuredHeight(), ANI_TIME/2);
        }
        invalidate();
    }
